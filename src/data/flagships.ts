...
export const flagshipProjects: FlagshipProject[] = [
  ...
  },
  {
    ...
    story:
      "CS 124 honors group project — five-person team for the honors track of UIUC's intro CS course. I led product and shipped the production cut: a hardened cross-origin session flow, an anti-alphabetical fix to the Wikipedia ranker, and a per-level prompt rework that lets each generated tree shape itself to the topic instead of falling out as a grid. Pick a concept and an LLM drafts a prerequisite learning tree; pick a Wikipedia article and a force-directed graph maps its conceptual neighborhood.",
    architecture: [
      "Two tools fused into one app — a Gemini-generated learning skill tree and a Wikipedia knowledge-graph explorer, sharing auth and a single design system",
      "Wikipedia ranker pulls 500 links per article and seed-shuffles to break the API's alphabetical bias — Quantum mechanics now surfaces Hilbert space, not Abdus Salam",
      "Skill-tree prompt engineered for asymmetric per-level shape: wide foundations, narrow convergence layers, a single apex; the tree silhouette varies with the topic",
      "Cross-origin session auth — secure cookies + CORS allowlist + trust-proxy across split frontend / backend origins",
      "d3-force layout with deterministic per-node positional jitter so trees feel organic, not grid-perfect",
      "Save / share / send-to-user flows backed by Supabase row-level CRUD; anonymous local trees auto-import on first sign-up",
    ],
    stats: [
      { label: "team size", value: "5" },
      { label: "course", value: "CS 124 honors" },
      { label: "wiki link pool", value: "500" },
      { label: "tree levels", value: "3–10" },
    ],
    demoKind: "iframe",
